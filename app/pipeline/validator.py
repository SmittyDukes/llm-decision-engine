...
def validate_output(parsed: dict):
    ...
    for field in required_fields:
        ...
        #2. Type checks
        if not isinstance(parsed["answer"], str) or not isinstance(parsed["reason"], str):
            return{
                "status": "fail",
                "failure_type": FAILURE_TYPES["INVALID_TYPE"],
                "reason": "Answer must be strings"
            }
        if parsed["reason"] is None or not isinstance(parsed["reason"], str) or not parsed["reason"].strip():
            return {
                "status": "fail",
                "failure_type": FAILURE_TYPES["FORMAT_DRIFT"],
        ...
        #4. Answer domain check (CRITICAL)
